...
class Ui_MainWindow(object):
    ...
    def buttonoutfile(self):
        ...
        temp_sqlstring1 = "select * from PSG_AJ_ where"
        if self.checkBox_2.isChecked():
            is_first1 = False
            if is_first:
                is_first = False
        ...
        if self.checkBox.isChecked():
            if is_first:
                is_first = False
                temp_sqlstring += " INTIME>=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss') + "and INTIME<=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit_2.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss')
            else:
                temp_sqlstring += " and INTIME>=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss') + "and INTIME<=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit_2.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss')
        ...
        if temp_sqlstring=="select * from PSG_AJ_OCR where":
            ...
            if is_first1:
                ...
                self.cursor.execute(temp_sqlstring1)
                data = self.cursor.execute(temp_sqlstring1)
                if data:
                    cols = []
                    for col in self.cursor.description:
                        cols.append(col[0])
