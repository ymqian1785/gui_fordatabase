...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_22.setText(_translate("MainWindow", "人像切图"))
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab), _translate("MainWindow", "实时"))
        self.label_20.setText(_translate("MainWindow", "图像"))
        self.groupBox_6.setTitle(_translate("MainWindow", "表一"))
        ...
        self.groupBox_7.setTitle(_translate("MainWindow", "表二"))
        ...
        self.tabWidget.setTabText(self.tabWidget.indexOf(self.tab_3), _translate("MainWindow", "客流分析"))
    ...
    def buttonsearch(self):
        ...
        temp_sqlstring4 = self.sqlstring4
        is_first = True
        is_first1 = True
        ...
        if is_first1:
            if temp_sqlstring =='select * from（select * from PSG_AJ_OCR order by NO desc）where rownum=1 AND':
                print('1')
                window = Dialog(self)
                window.show()
            else:
            ...
            if temp_sqlstring1 =="select * from（select * from PSG_AJ_ZC order by ID desc）where rownum=1 AND and IDCARD=''":
                print('1')
                window = Dialog3(self)
                window.show()
            else:
    ...
    def buttonoutfile(self):
        is_first = True
        temp_sqlstring = "select * from PSG_AJ_OCR e,PSG_AJ_ZC d where"

        if self.checkBox_2.isChecked():
            if is_first:
                is_first = False
                temp_sqlstring += " d.MACHINENO='%s'" % self.lineEdit_21.text()
            else:
                temp_sqlstring += " AND d.MACHINENO='%s'" % self.lineEdit_21.text()

        if self.checkBox.isChecked():
            if is_first:
                is_first = False
                temp_sqlstring += " d.INTIME>=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss') + " and d.INTIME<=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit_2.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss')
            else:
                temp_sqlstring += " AND d.INTIME>=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss') + " AND d,INTIME<=to_date('%s','yyyy-mm-dd hh24:mi:ss')" % self.dateTimeEdit_2.dateTime().toString(
                    'yyyy-MM-dd hh:mm:ss')
        ...
        self.cursor.execute(temp_sqlstring)
        data = self.cursor.execute(temp_sqlstring)
        if data:
            cols = []
...
class Dialog2(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.pwd.setAlignment(Qt.AlignVCenter)  # 设置QLabel居中显示
...
class Dialog4(QtWidgets.QDialog):
    def __init__(self, parent=None):
        ...
        self.pwd = QtWidgets.QLabel(self)
        self.pwd.setAlignment(Qt.AlignCenter)  # 设置QLabel居中显示
        self.pwd.setText(u'导出出错')

class Dialog5(QtWidgets.QDialog):
    def __init__(self, parent=None):
        QtWidgets.QDialog.__init__(self, parent)
        self.setWindowTitle("打开网页例子")
        # 相当于初始化这个加载web的控件
        self.view = QWebEngineView()
        self.view.load(QUrl("http://10.1.50.203/"))
        self.view.show()
